...
describe('css sanitizer builder test', () => {
  ...
  it('build builds a CSS Sanitizer', () => {
    const sanitizer = new CssSanitizerBuilder().build();
    expect(() => sanitizer.sanitizeToFragment('<div></div>')).not.toThrow();
  });
  ...
  describe('sanitizeToFragment', () => {
    ...
    it('adds internal CSS to the shadow DOM', () => {
      // Note that we test the exact contents of the CSS is not checked so
      // that this test doesn't become a "change detector".
      const sanitizer = new CssSanitizerBuilder().build();
